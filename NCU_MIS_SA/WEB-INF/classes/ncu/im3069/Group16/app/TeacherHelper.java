...
public class TeacherHelper {
  ...
  public JSONObject getByID(String id) {		//1212 4pm by min
      ...
      try {
          ...
          conn = DBMgr.getConnection();
          /** SQL指令 */
          String sql = "SELECT * FROM `sa16`.`teachers` WHERE `id` = ? LIMIT 1";
  ...
  //
  public int getIDByEmail(String email) {		//1228 2am by min
      //回傳找到的id
      int id=0;
      /** 記錄實際執行之SQL指令 */
      String exexcute_sql = "";
      ...
      try {
          ...
          conn = DBMgr.getConnection();
          /** SQL指令 */
          String sql = "SELECT `id` FROM `sa16`.`teachers` WHERE `email` = ? LIMIT 1";
  ...
  public JSONObject updateLogin(String email, boolean login) {	//1223 9pm, by min
      ...
      try {
          ...
          conn = DBMgr.getConnection();
          /** SQL指令 */
          String sql = "Update `sa16`.`teachers` SET `login` = ? WHERE `email` = ?";
  ...
  public String whoLogIn() {
      ...
      try {
          ...
          conn = DBMgr.getConnection();
          /** SQL指令 */
          String sql = "SELECT `id` FROM `sa16`.`teachers` WHERE `login` = ?";
  ...
  public boolean checkLogin(String email,  String password){	//1228 12pm
      /** 紀錄SQL總行數，若為「-1」代表資料庫檢索尚未完成 */
      int row = -1;
      /** 儲存JDBC檢索資料庫後回傳之結果，以 pointer 方式移動到下一筆資料 */
      ResultSet rs = null;
      ...
      try {
          ...
          conn = DBMgr.getConnection();
          /** SQL指令 */
          String sql = "SELECT `id` FROM `sa16`.`teachers` WHERE `email` = ? and `password` = ?";
